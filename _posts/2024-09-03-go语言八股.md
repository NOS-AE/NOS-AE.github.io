...
如果缓冲区不存在数据，使用`recvDirect`直接从发送者`sudog`那里拿，否则拿缓冲区的数据，然后将发送者的数据放到缓冲区中。最终都会调用`goready`将发送方goroutine设置为可运行状态，在调度器下一次调度将其唤醒。
...
## 🔗
 ...
 https://www.bilibili.com/video/BV1hv411x7we/?p=4&share_source=copy_web&vd_source=c0d41b92058c38bc23233feb8c73c581
