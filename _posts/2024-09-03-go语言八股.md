...
![图片](https://cdn.jsdelivr.net/gh/NOS-AE/assets@main/img/640)
...
golang程序代码的运行离不开进程、线程以及goroutine，所以我打算从整个程序的第一行汇编指令的执行开始说起，因为我们经常说到的g0、m0其实是在汇编代码中初始化的。不过不会太深入，因为很多知识我也只是有个概念，欢迎指出谬误。
...
1. 初始化启动参数
2. 初始化g0和m0结构体，并将他们绑定
3. 初始化调度器
4. 创建一个新的g去运行runtime.main
5. 开始循环调度

我们主要关注初始化调度器、如何创建g、如何循环调度。首先是`runtime.schedinit`：
