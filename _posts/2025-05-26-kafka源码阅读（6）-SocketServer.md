...
本章开始上点干货，真正看一下 kafka 的网络层是如何运作的，主要是解析 `SocketServer` 这个核心类。本篇的解析重点在于 SocketServer 中的 Acceptor 以及 Processor 线程。
...
Acceptor 作为 reactor 模式中 dispatcher 的角色，负责接收网络请求事件然后分发给下面的线程去处理。看一下类定义：
...
`Acceptor` 成员中的的 `sendBufferSize` 和 `recvBufferSize` 可以分别通过 broker 端的 `socket.send.buffer.bytes` 和 `socket.receive.buffer.bytes` 参数进行配置，默认是 100KB。另外说个题外话，这里引出了一个生产环境的调优点：在生产环境中，如果客户端和 broker 的网络通信延迟比较大（比如RTT>10ms），这样一来带宽延迟乘积（Bandwidth delay product, BDP）就比较大，那么可以将他们的两个缓冲区大小调高些，以提高数据吞吐量，因为 BDP 表示了网络承载能力（想象一下网络是一根水管，带宽是每秒的水量，延迟是端到端的延迟，那么 BDP 就是已发出但还未到达对端的总水量），最大接收窗口就表示了网络承载能力内可以不经确认发出的报文。
...
可以看到移除 processor 将会调用 `shutdown` 关闭 processor。但里面涉及到一些一时半会也解释不清的东西，包括 `wakeup` 还有 `shutdownLatch`，后续遇到再进行分析。

上面简单分析了 `Acceptor` 对 processor 的管理相关方法，但作为一个 `Runnable`，我们重点需要分析的是 `run` 方法，即线程执行的代码：

``` scala
def run(): Unit = {
  // 注册accept事件
  serverChannel.register(nioSelector, SelectionKey.OP_ACCEPT)
  // Acceptor正常启动完毕，主要是对CountDownLatch的操作
  startupComplete()
  try {
    ...
    // Acceptor正常关闭完毕，主要是对CountDownLatch的操作
    shutdownComplete()
  }
}
...
最后，客户端 IP 对应的连接数超过最大限制时，为了防止客户端滥用，会抛 `TooManyConnectionsException` 异常，拒绝连接。
...
- `inflightResponses` 不同于 `responseQueue`，后者是为了排队这些待发送的响应，而有些 Response 回调函数需要在响应发送给客户端之后才触发，因此额外需要一个 `inflightResponses` 来暂存正在发送的响应。

接下来我们继续分析 `Processor` 线程的方法。`Processor` 的众多方法都是由 `run` 方法展开调用的，因此我们直接分析 `run` 方法：

``` scala
override def run(): Unit = {
  // Processor正常启动完毕，主要是对CountDownLatch的操作
  startupComplete()
  try {
    while (isRunning) {
      try {
    ...
    // Processor正常关闭完毕，主要是对CountDownLatch的操作
    shutdownComplete()
  }
}
```
...
private def configureNewConnections(): Unit = {
  ...
  while (connectionsProcessed < connectionQueueSize && !newConnections.isEmpty) {
    ...
    try {
      ...
      // 将该新连接交由kselector处理，实际上是注册socket的读事件
...
接下来是`processNewResponses`，这个方法处理响应队列中的响应：
    ...
    try {
      currentResponse match {
        case response: NoOpResponse =>
          ...
          // 无操作响应，服务器应获取更多更多Request请求（反正也没事干），因此要取消通道静音
...
在 `processNewResponses` 中的几种响应类型在上一篇博客中已经说过，不再多说。另外这里出现了一些陌生的概念，比如 通道 `KafkaChannel`、静音 `mute` 等，通道用来封装一个网络连接，隐藏了底层的网络通信细节，而静音和限流都是通道提供的功能，静音期间，服务器不会从该通道读取更多请求，但会继续处理已接收的请求。这些概念目前知道大概的意思就够了。
