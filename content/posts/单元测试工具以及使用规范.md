...
- miniredis（redis mock 库）：用于本地启动一个基于内存的 redis 实例，与 go-redis 无缝集成。

其中，Ginkgo + Gomega 让我们能以 DSL 的形式编写测试用例。testify 本身包含了断言和 mock，为了统一风格，我们仅使用 Gomega 进行断言，mock 则交给 testify/mock 和 gomonkey。
...
实际上 miniredis 无需你手动 mock，而是基于内存直接 mock 了一个 redis 服务器给你用。这样一来，除了无需手动 mock 之外，甚至与任何 redis 客户端都能无痛集成，无论是对单元测试还是集成测试都极其友好。下面是使用方式：

直接对 miniredis 对象端进行 redis 读写：
...
一般情况下，我们更希望与 redis 客户端框架集成，比如 go-redis：
...
对于 TTL，miniredis 不会真的去计算已经过了多少秒，而是通过 `FastForward` 模拟时间的流逝：
