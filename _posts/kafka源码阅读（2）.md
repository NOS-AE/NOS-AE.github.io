...
class Log(
  ...
  // high watermark高水位，已提交与未提交消息的分隔线
...
<img src="https://cdn.jsdelivr.net/gh/NOS-AE/assets@main/img/image-20250429200644249.png" alt="image-20250429200644249" style="zoom:50%;" />

如图所示，从偏移量 3 开始的消息都对用户可见（可访问），从偏移量 8 开始的消息都未提交（uncommited），下一条待插入消息的偏移量为 15。另外，你可能还听说过 LSO 这个偏移量，但它并不是 log start offset，而是 kafka 事务中存在的概念 log stable offset，暂时不需要理会。

接下来我们开始分析 Log 类的初始化都干了些什么。首先 Log 类中有一个 `locally` 代码块，关于 `locally` 本身的作用在此不多讲，这块代码将会在构造 Log 类的时候被执行：
...
可以看到 Log 初始化还是干了不少事情，下面还是会以 DFS 的方式一步步去查看各个方法进行分析。首先创建文件夹不多说，其次 leader epoch cache 暂时不知道干嘛的，也跳过。直接看到 `loadSegments` 这个方法，这个方法负责加载日志段，核心就是构造相应的 LogSegmemt 对象。

具体地来说，`loadSegments`会删除一些无效文件比如 .deleted 文件、无效的 .swap 文件等，随后会将有效的 .swap 文件恢复成 .log 文件，最终将所有 .log 文件构造出在内存中相应的 LogSegment 对象。
