...
首先要基本了解kafka的基本结构，一个日志（Log）代表一个 kafka 分区（Partition）的数据存储实现，结合下图进行理解，每个日志包含多个分段（Segment），每个分段由磁盘上的一组文件来表示，包括最重要的数据文件（.log）以及索引文件（.index），还包含了用于其它功能的时间戳索引文件（.timeindex），如果使用了kafka事务还会包含事务相关索引文件（.txnindex）。
...
索引文件非常重要，当我们通过offset来查找消息时，势必要遍历log文件中的所有数据，而索引文件通过类似跳表那样的稀疏索引机制，来做到一次查找可以跳过许多不必要的读数据操作，但索引文件又没有跳表那么复杂，可以简单看成是只有两层的跳表，上层是 index，下层是 log，其索引方式大致如图所示：
