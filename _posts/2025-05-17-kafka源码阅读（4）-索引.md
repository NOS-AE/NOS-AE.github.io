...
本篇开篇特别介绍了 kafka 对索引项查找的相关优化，主要是通过冷热分区，让多数的访问集中在索引文件的后几页，充分适应操作系统 page cache 的 LRU 淘汰算法，减少了缺页 I/O 带来的运行效率下降问题。随后介绍了偏移量索引以及时间戳索引的实现，最后提了一嘴用于懒加载索引的 LazyIndex。
