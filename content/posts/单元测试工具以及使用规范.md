...
> [!tip]
>
> 测试工具比如 ginkgo 提供了许多看起来很 fancy 很有用的函数，比如 `AroundNode` 节点、各种装饰器等，但它们同时也增加了别人对代码的理解成本，请遵循 KISS 原则，切勿滥用。
...
我们可以在代码中通过 `GinkgoRandomSeed` 获取，并将其作为我们随机数生成器的种子。这样的好处在于，当测试失败的时候，下次运行时加上 `--seed=1634748172` 选项将种子指定为上次种子，来稳定地复现失败结果。

其次，ginkgo 也会依赖这个种子，将 UT 的运行顺序打乱，有助于发现我们是否破坏了测试用例之间的独立性。因此将种子固定，能让 UT 运行顺序与上次相同，来稳定地复现失败结果。

### 随机数种子

我们还能使用在程序中 `GinkgoRandomSeed` 函数获取上面提到的这个随机种子，并用它来初始化我们的随机数生成器。这样一来当 UT 失败的时候，我们也只需要通过 `--seed` 传入上一次的随机种子来复现失败结果。（传统做法是：在程序中生成种子、打印出来、UT 失败、手动硬编码代码中的种子为上次的种子）。
...
> [!CAUTION]
>
> 非必要的情况下不要使用 gomonkey，如果发现不得不用的情况，请改造代码。如果是临时使用，用完即删
