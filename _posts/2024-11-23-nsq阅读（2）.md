...
上一篇中，主要对nsq中涉及到的主要组件，以及数据流在这些组件中的流动进行了简单说明。在本篇中，我们跟随上一篇最后给出的demo，对应到nsqd的代码中，看一下执行流程是怎么样的。
...
func (n *NSQD) GetTopic(topicName string) *Topic {
    ...
    // 使用nsqlookupd发现channel并创建channel
...
创建topic的时候会启动topic的messagePump协程，监听topic的消息投递事件，然后将消息投递到所有绑定的channel中。
...
综上，当我们发送消息的时候，`doPUB`这个handler会将消息投递到topic的内存队列`topic.memoryMsgChan`，满了的话就投递到磁盘队列`t.backend`中。随后在`topic.messagePump`监听到消息的投递，并投递到与topic绑定的所有channel的内存队列或磁盘队列中。
